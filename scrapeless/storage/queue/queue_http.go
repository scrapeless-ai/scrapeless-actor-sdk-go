...
func NewQueueHttp(queueId ...string) Queue {
	log.GetLogger().Info().Msg("queue http init")
...
func (q *QueueHttp) List(ctx context.Context, page int64, pageSize int64, desc bool) (*ListQueuesResponse, error) {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to list queues: %v\n", code.Format(err))
...
func (q *QueueHttp) Create(ctx context.Context, req *CreateQueueReq) (queueId string, queueName string, err error) {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to create queue: %v\n", code.Format(err))
...
func (q *QueueHttp) Get(ctx context.Context, name string) (*Item, error) {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to get queue: %v\n", code.Format(err))
		return nil, code.Format(err)
	}
	return &Item{
		Id:          queue.Id,
...
func (q *QueueHttp) Delete(ctx context.Context) error {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to delete queue: %v\n", code.Format(err))
		return code.Format(err)
	}
	return nil
}
...
func (q *QueueHttp) pushWithId(ctx context.Context, req PushQueue) (string, error) {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to push to queue: %v\n", code.Format(err))
		return "", code.Format(err)
	}
...
func (q *QueueHttp) pullWithId(ctx context.Context, size int32) (GetMsgResponse, error) {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to pull from queue: %v\n", code.Format(err))
		return nil, code.Format(err)
	}
	if msgs == nil {
		return nil, nil
...
func (q *QueueHttp) ackWithId(ctx context.Context, msgId string) error {
	...
	if err != nil {
		log.GetLogger().Error().Msgf("failed to ack msg: %v\n", code.Format(err))
		return code.Format(err)
	}
	return nil
}
